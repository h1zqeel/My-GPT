...
export default function Chat({ id }: TChatProps) {
	...
	const messages = useAppSelector(
		({ messagesReducer }) => messagesReducer.messages
	);
	const messagesLoading = useAppSelector(
		({ messagesReducer }) => messagesReducer.loading
	);
	const messagesError = useAppSelector(
		({ messagesReducer }) => messagesReducer.error
	);
	...
	useEffect(() => {
		if (completion.length > 1) {
			dispatch(
				updateBotMessage({ role: 'assistant', content: completion })
			);
			messagesEndRef.current?.scrollIntoView({ behavior: 'instant' });
		}
	}, [completion]);
	...
	const allowSubmit = useMemo(() => {
		...
		if (
			!user?.googleAIKey &&
			(currentChat?.llm === 'googlepalm' ||
				currentChat?.llm === 'googlegemini')
		) {
			return false;
		}
			...
			<div className="flex flex-col h-[calc(100dvh)] overflow-clip w-[100vw] lg:w-[100%] pt-20 md:pt-0 lg:pt-0">
				<div
					className={`grow justify-center ${
						messagesLoading ? 'overflow-clip' : 'overflow-scroll'
					}`}
				>
					{!messagesLoading &&
