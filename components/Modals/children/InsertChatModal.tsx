...
export const InsertChatModal = ({ props, handleClose } : TInsertChatModal) => {
	...
	return <>
		...
		{!props?.chatId && <>
			{loading ? <div className='float-right'><CircularProgress size={25} /> </div>: <Button variant='outlined' color='primary' disabled={(fetchingEngines || preventChatCreation || loading || !allowedEngines.length)} className='float-right' onClick={handleChatCreation}>Create</Button>}
